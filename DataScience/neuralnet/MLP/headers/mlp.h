...
class MLP{
    ...
    public:
        ...
        void reset_weights(uint start_layer=1, uint end_layer=__INT_MAX__, double factor=1.0); // manually reset weights (note: weights are automatically set whenever a layer is added)
        void feedforward(uint start_layer=1, uint end_layer=__INT_MAX__);
        void backpropagate();
        void set_input(uint index, double value); // set single input via 1d index
        double get_input(uint index, uint l=0){return layer[l].neuron[index].x;}
        double get_output(uint index); // set single output via 1d index        
        double get_hidden(uint index,uint layer_index); // get a single 'h' from a hidden layer via index
        void set_label(uint index, double value); // set a single label value via 1d output neuron index
