...
class Sample{
    private:
        ...
        vector<double> coefficients; //for polynomial regression
    ...
    public:
        ...
        double median(){
            // make a copy
            vector<double>sorted_copy = this->sort();
        ...
        // weighted average of a 1d double vector
        double weighted_average(bool as_series=false){
          ...
          // ranking 
          vector<int> ranking(bool low_to_high=true){
              // initialize ranks
              vector<int> rank(elements);
              for (int n=0;n<elements;n++){
                  rank[n]=n;
              }
        ...
        vector<double> exponential_smoothing(bool as_series=false){
            double alpha=2/(elements);
            vector<double> result(elements);
        ...
        double stddev(vector<double>& sample){
        ...
        int find(double value,int index_from=0,int index_to=__INT_MAX__){
            int findings=0;
        ...
        double Dickey_Fuller(){
            // make a copy
            vector<double> data_copy = data_vect;
            vector<double> diff = Sample(data_copy).stationary(integer,1);
        ...
        void correlation(){
            ...
            vector<int> rank_x = Sample(x_vect).ranking();
            vector<int> rank_y = Sample(y_vect).ranking();
            double numerator=0;
            for (int n=0;n<elements;n++){
        ...
        vector<double> stationary(DIFFERENCING method=integer,double degree=1,double fract_exponent=2){
            // make a copy
            vector<double> target = data_vect;
            if (method==integer){
        ...
        // vector sort via pairwise comparison
        vector<double> sort(bool ascending=true){
            // make a copy
            vector<double> data_copy = data_vect;
        ...
        // random shuffle (for 1-dimensional type <double> vectors)
        vector<double> shuffle(){
            vector<double> result(elements); //=make a copy
            for (int i=0;i<elements;i++){
                result[i]=data_vect[i];
            }
            for (int i=0;i<elements;i++){
                int new_position=floor(rand_uni()*elements);
                double temp=data_vect[new_position];
                result[new_position]=data_vect[i];
                data_vect[i]=temp;
            }
        }
        // log transformation
        vector<double> log_transform(){
            vector<double> target(elements);
            for (int n=0;n<elements;n++){
        ...
        double Engle_Granger(){
            vector<double> log_x(elements), log_y(elements);
            for (int i=0;i<elements;i++){
                log_x[i]=log(x_vect[i]);
                log_y[i]=log(y_vect[i]);
            }
            Sample* res = new Sample(log_x,log_y);
        ...
        double polynomial_predict(double x) {
        ...
        void polynomial_regression(int power =5) {
            ...
            vector<vector<double>> X(elements, vector<double>(power + 1));
            for (int i = 0; i < elements; i++) {
        ...
        Sample(){};
        // parametric constructor for single vector-type sample
        Sample(vector<double> data_vect){
            this->data_vect = data_vect;
            elements = data_vect.size();
        }
        // parametric constructor for two vector-type samples
        Sample(vector<double> x_vect, vector<double> y_vect){
            this->x_vect = x_vect;
            this->y_vect = y_vect;
            elements = fmin(x_vect.size(),y_vect.size());
        }       
        // parametric constructor for single array-type sample
        Sample(double *data_vect){
            elements = sizeof(data_vect)/__SIZEOF_DOUBLE__;
            for (int i=0;i<elements;i++){
                this->data_vect.push_back(data_vect[i]);
            }
        }
        // parametric constructor for two array-type samples
        Sample(double *x_vect, double *y_vect){
            elements = fmin(sizeof(x_vect)/__SIZEOF_DOUBLE__,sizeof(y_vect)/__SIZEOF_DOUBLE__);
            for (int i=0;i<elements;i++){
