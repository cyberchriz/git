...
double f_Xavier_normal(int fan_in, int fan_out){
    double result=rand_norm(0.0,1.0); // get a random number from a normal distribution with zero mean and variance one
...
double f_Xavier_uniform(int fan_in, int fan_out){
    double result=rand_uni(0.0,1.0);
...
double f_He_ELU(int fan_in){
    double result=rand_norm(0.0,1.0);
